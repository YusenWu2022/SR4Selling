...
# 全局变量
use_llm = True
model, tokenizer = init_ds_model(device_id=1)
...
Additionally, here are some similar historical decisions and reflections for reference:
{similar_memories}
"""
...
# 4. 记忆模块管理
class MemoryModule:
    def __init__(self, memory_file='memory_module.json'):
    ...
    def add_memory(self, iteration, model_function, mae, mse, reflections):
        # 计算函数哈希作为唯一ID
        func_hash = hashlib.md5(model_function.encode()).hexdigest()
    ...
    def find_similar_memories(self, current_function, top_n=5):
        """使用TF-IDF和余弦相似度查找相似函数"""
        if not self.memories:
            return []
        
        # 准备文本数据
        functions = [mem['model_function'] for mem in self.memories]
        functions.append(current_function)
        
        # 创建TF-IDF向量器
        vectorizer = TfidfVectorizer().fit(functions)
        vectors = vectorizer.transform(functions)
        ...
        # 计算余弦相似度
        similarities = cosine_similarity(current_vector, memory_vectors)[0]
        
        # 获取最相似的top_n个记忆
        sorted_indices = np.argsort(similarities)[::-1][:top_n]
        similar_memories = [self.memories[i] for i in sorted_indices]
        
        return similar_memories
    
    def format_similar_memories(self, similar_memories):
        ...
        for i, mem in enumerate(similar_memories):
            formatted += f"\n==== Similar Memory #{i+1} ====\n"
            formatted += f"Iteration: {mem['iteration']}\n"
            formatted += f"MAE: {mem['mae']:.4f}, MSE: {mem['mse']:.4f}\n"
            formatted += "Reflections:\n"
            if isinstance(mem['reflections'], dict):
                for key, value in mem['reflections'].items():
                ...
                formatted += f"{mem['reflections']}\n"
            
            formatted += "Function Snippet:\n"
            func_lines = mem['model_function'].split('\n')
            snippet = '\n'.join(func_lines[:5] + ['...'] + func_lines[-3:]) if len(func_lines) > 8 else mem['model_function']
            formatted += snippet + "\n"
            formatted += "="*40 + "\n"
...
# 7. 决策树优化主流程
def decision_tree_optimization(df, target_col, threshold=5, max_iterations=5):
    # 创建TensorBoard写入器
    timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    log_dir = os.path.join("runs", f"only_llm_memory_{timestamp}")
    writer = SummaryWriter(log_dir=log_dir)
    print(f"TensorBoard logs will be saved to: {log_dir}")
    
    # 初始化记忆模块
    memory_module = MemoryModule(memory_file=log_dir+"/memory_module.json")
    ...
    while current_iteration < max_iterations:
        ...
        while not success and retry_count < max_retries:
            try:
                if current_iteration == 0:
                    ...
                    similar_memories = []
                    if previous_model:
                        similar_memories = memory_module.find_similar_memories(previous_model, top_n=5)
                    similar_memories_text = memory_module.format_similar_memories(similar_memories)
        ...
        # 添加到记忆模块
        memory_module.add_memory(current_iteration, model_function_str, mae, mse, reflections)
